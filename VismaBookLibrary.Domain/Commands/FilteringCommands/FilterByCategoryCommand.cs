...
{
    public class FilterByCategoryCommand : IFilterCommand
    {
        private readonly IFileService _fileService;
        private readonly IValidationService _validationService;

        public FilterByCategoryCommand(IFileService fileService, IValidationService validationService)
        {
        ...
        public void Execute(string option, string phrase)
        {
            var filteredTaken = _fileService.GetAll().Where(b => b.TakenBy != null)
                .Where(b => b.Category.Contains(phrase, StringComparison.OrdinalIgnoreCase));

            var filteredAvailable = _fileService.GetAll().Where(b => b.TakenBy == null)
                .Where(b => b.Category.Contains(phrase, StringComparison.OrdinalIgnoreCase));

            var filteredAll = _fileService.GetAll().Where(b => b.Category.Contains(phrase, StringComparison.OrdinalIgnoreCase));
